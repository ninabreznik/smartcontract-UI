...
async function makeReturn (css, solcMetadata, provider, transaction, fnName) {
  let types = returnTypes(solcMetadata.output.abi, fnName)
  let decodedTx
  ...
  var el = bel`
    <div class=${css.txReturnItem}>
      <div class=${css.txReturnField}>
        <div class=${css.txReturnTitle}>Created:</div>
        <div class=${css.txReturnValue}>${date()}</div>
      </div>
    </div>`
  if (transaction.hash) el.appendChild(moreInfo(css, provider, transaction))
  el.appendChild(bel`<div class=${css.returnJSON}><pre>${JSON.stringify(decodedTx, 0, 2)}</pre></div>`)
  return el
}
...
function moreInfo (css, provider, transaction) {
  ...
  return bel`<div class=${css.infoIcon}><a href=${linkToEtherscan} target="_blank"><i class="fa fa-info-circle"></i></a></div>`
}
